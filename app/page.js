...
export default function TokitokiPrototype() {
  ...
  return (
    <div className="min-h-screen bg-[#f7f4ee] pb-28 text-zinc-950 lg:pb-0">
...
function ProductCard({ product, qty, setQty, addToCart }) {
  ...
  return (
    <article className="group overflow-hidden rounded-[2rem] bg-white shadow-sm ring-1 ring-stone-200 transition hover:-translate-y-1 hover:shadow-xl hover:shadow-stone-300/40">
      <div className="relative h-48 overflow-hidden"><img className="h-full w-full object-cover transition duration-500 group-hover:scale-105" src={product.image} alt={product.name} /><div className="absolute left-3 top-3 rounded-full bg-white/95 px-3 py-1 text-xs font-black text-emerald-800 shadow">{product.subcategory}</div></div>
      <div className="p-4">
