...
export default function TokitokiPrototype() {
  ...
  const subcategories = [
    ...new Set(
      products.filter((p) => p.type === selectedType).map((p) => p.subcategory)
    ),
  ];
  ...
  function updateCartQty(id, nextQty) {
    ...
    if (!value || value < 1) {
      setCart((prev) =>
        prev.map((i) => (i.id === id ? { ...i, qty: 1 } : i))
      );
      return;
    }

    setCart((prev) =>
      prev.map((i) => (i.id === id ? { ...i, qty: value } : i))
    );
  }
  ...
  function removeFromCart(id) {
    ...
    const confirmed = window.confirm(
      `Usunąć z koszyka: ${item?.name || "ten produkt"}?`
    );
    if (!confirmed) return;
  ...
  function validateDeliveryAndGoNext() {
    if (!zoneId)
      return alert("Wybierz miejsce dostawy: Rzeszów albo okolice Rzeszowa.");
    goToStep(4);
  }
  ...
  async function sendOrder() {
    ...
    try {
      if (!zoneId) return alert("Wybierz miejsce dostawy.");
      if (!isPhoneValid)
        return alert("Podaj poprawny numer telefonu — 9 cyfr.");
      if (!isAddressValid) return alert("Podaj dokładny adres dostawy.");
      if (cart.length === 0)
  ...
  return (
    <div className="min-h-screen bg-[#f7f4ee] pb-28 text-zinc-950 lg:pb-0 [&_a]:cursor-pointer [&_button]:cursor-pointer">
      ...
      <section className="relative overflow-hidden bg-gradient-to-br from-white via-stone-50 to-emerald-50/60">
        <div className="mx-auto grid max-w-7xl gap-8 px-4 pb-8 pt-7 lg:grid-cols-[0.95fr_1.05fr] lg:px-8 lg:py-16">
          ...
          >
            ...
            <p className="mt-5 max-w-2xl text-base leading-7 text-zinc-700 sm:text-lg">
              Wybierz materiał, podaj ilość i adres. My to załadujemy na nasze
              "taczki" i przywieziemy :) !
            </p>
            ...
            <div className="mt-7 grid gap-3 sm:flex sm:flex-wrap">
              ...
              <a
                ...
                className="inline-flex min-h-14 items-center justify-center gap-3 rounded-2xl border-2 border-emerald-800 bg-white px-7 py-4 text-base font-black text-emerald-900 transition hover:bg-emerald-50"
              >
          ...
          <div className="relative min-h-[300px] overflow-hidden rounded-[2rem] bg-stone-200 shadow-2xl shadow-stone-300/60 lg:min-h-[520px]">
            <img
              ...
              alt="Dostawa kruszyw w Rzeszowie"
            />
      ...
      <main id="zamowienie" className="scroll-mt-28 mx-auto max-w-7xl px-4 py-8 lg:px-8">
        <OrderProgress step={step} goToStep={goToStep} />
          ...
          >
            {step === 1 && (
              <Panel>
                <SectionIntro
                  ...
                  text="Wybierz kategorię — potem pokażemy tylko pasujące produkty."
                />
                    ...
                    <div className="mt-4 flex gap-3 overflow-x-auto pb-1">
                      {subcategories.map((s) => (
                        <button
                          ...
                          className={`min-h-12 shrink-0 rounded-2xl px-5 font-black transition ${
                            subcategory === s
                ...
                <div id="produkty-lista" className="scroll-mt-28 flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
                  <div>
                    ...
                    <h2 className="text-3xl font-black tracking-tight">
                      {selectedType === "build" ? subcategory : "Kamienie ozdobne"}
                    </h2>
                  </div>
                  <p className="text-sm font-bold text-zinc-500">
                    Kliknij „Dodaj” — koszyk pojawi się na dole ekranu.
                  </p>
                </div>
            ...
            {step === 3 && (
              <section className="space-y-6">
                <Panel>
                  <SectionIntro
                    eyebrow="Krok 3"
                    title="Gdzie dostarczyć?"
                    text="Cena transportu naliczy się automatycznie dla Rzeszowa albo przejdzie do wyceny indywidualnej."
                  />
                    ...
                    {deliveryZones.map((z) => {
                      ...
                      return (
                        <button
                          ...
                          className={`flex min-h-20 w-full items-center justify-between gap-4 rounded-3xl border p-4 text-left transition ${
                            zoneId === z.id
                        ...
                        >
                          <span>
                            <span className="flex items-center gap-2 text-lg font-black">
                              {z.name}
                              {z.id === "area" && <HelpCircle size={17} className="text-zinc-400" />}
                            </span>
                            <span className="mt-1 block text-sm text-zinc-500">{z.note}</span>
                          </span>
                          <span className="rounded-2xl bg-white px-4 py-2 font-black text-emerald-800 shadow-sm">
                            {displayPrice}
                          </span>
                        </button>
                      );
                  ...
                  <div className="mt-5 rounded-3xl bg-emerald-50 p-4 text-sm text-emerald-950">
                    <div className="flex items-start gap-3">
                      <Gift size={22} className="mt-0.5 shrink-0" />
                      <div>
                        <b>Transport gratis na terenie Rzeszowa</b>
                        <p className="mt-1 text-emerald-800">
                          Przy zamówieniu powyżej 2000 zł netto transport naliczy się jako 0 zł.
                        </p>
                      </div>
                    </div>
                  </div>
                ...
                <StepActions
                  backLabel="Wstecz"
                  nextLabel="Dalej: dane"
                  onBack={() => goToStep(2)}
                  onNext={validateDeliveryAndGoNext}
                />
              </section>
            )}
            ...
            {step === 4 && (
              <Panel>
                <SectionIntro
                  eyebrow="Krok 4"
                  title="Dane do potwierdzenia"
                  text="Nie pobieramy płatności online. Najpierw potwierdzimy telefonicznie szczegóły zamówienia."
                />
                <div className="mt-5 grid gap-4 md:grid-cols-2">
                  <Input
                  ...
                  <div className="md:col-span-2">
                    <Input
                      ...
                      error={form.address && !isAddressValid}
                    />
                  ...
                  <div className="rounded-3xl border border-stone-200 bg-white p-4 md:col-span-2">
                    ...
                    <div className="flex flex-wrap gap-3">
                      <DateButton
                        active={form.date === todayDate()}
                        onClick={() => setForm({ ...form, date: todayDate() })}
                      >
                        Dziś
                      </DateButton>
                      <DateButton
                        active={form.date === tomorrowDate()}
                      ...
                      <input
                        type="date"
                        className="min-h-12 rounded-2xl border border-stone-200 px-4 font-bold"
                        value={form.date}
                  ...
                  />
                  <textarea
                    className="min-h-32 rounded-3xl border border-stone-200 bg-white p-4 outline-none transition focus:border-emerald-700 focus:ring-4 focus:ring-emerald-100 md:col-span-2"
                    placeholder="Uwagi: wąski wjazd, brak miejsca do zawracania, ograniczenia tonażowe..."
                    value={form.notes}
                ...
                <StepActions
                  ...
                  onBack={() => goToStep(3)}
                  onNext={validateAndSendToSummary}
                />
              </Panel>
            )}
            ...
            {step === 5 && (
              <section className="mx-auto max-w-3xl">
                <Panel>
                  <SectionIntro
                    eyebrow="Krok 5"
                    title="Sprawdź i wyślij zamówienie"
                    text="Po wysłaniu oddzwonimy, żeby potwierdzić dostępność, transport i termin."
                  />
                  ...
                  <div className="mt-6 grid gap-3 md:grid-cols-2">
                    <button
                      type="button"
                      onClick={() => goToStep(2)}
                      className="inline-flex min-h-14 items-center justify-center gap-2 rounded-2xl bg-stone-100 px-6 font-black text-zinc-700"
                    >
      ...
      <section className="mx-auto max-w-7xl px-4 py-10 lg:px-8">
        <div className="rounded-[2rem] bg-white p-6 shadow-sm ring-1 ring-stone-200">
          <h2 className="text-2xl font-black">Kruszywa Rzeszów – sprzedaż i transport</h2>
    ...
    <header className="sticky top-0 z-40 border-b border-stone-200 bg-white/90 backdrop-blur-xl">
      <div className="mx-auto flex max-w-7xl items-center justify-between gap-3 px-4 py-3 lg:px-8">
        ...
        >
          <LogoMark />
          <div>
            <div className="text-xl font-black leading-none tracking-tight sm:text-2xl">TOKITOKI</div>
        ...
        <div className="flex items-center gap-2">
          ...
          <a
            ...
            href={`tel:${PHONE}`}
          >
            <Phone size={16} /> <span className="hidden sm:inline">{PHONE_LABEL}</span>
          </a>
        </div>
      </div>
    </header>
      ...
      <div className="rounded-[2rem] bg-white p-5 shadow-sm ring-1 ring-stone-200">
        ...
        >
          <div className="flex items-start gap-3">
            ...
            <div>
              <h2 className="text-2xl font-black">Nie wiesz ile zamówić?</h2>
              <p className="mt-1 text-sm text-zinc-600">
                Policz orientacyjną ilość materiału przed dodaniem do koszyka.
              </p>
            </div>
          </div>
          <span className="inline-flex items-center gap-1 rounded-xl bg-emerald-800 px-4 py-2 text-sm font-black text-white">
            {showCalc ? "Zwiń" : "Rozwiń"}
            <ChevronDown size={16} className={showCalc ? "rotate-180" : ""} />
          </span>
        </button>
        <AnimatePresence>
          {showCalc && (
            ...
            >
              ...
              <div className="mt-6">
                <p className="font-black">Czy materiał będzie zagęszczany?</p>
                <div className="mt-3 flex flex-wrap gap-3">
                  <DateButton active={calc.compaction === 1} onClick={() => setCalc({ ...calc, compaction: 1 })}>
                    Nie
                  </DateButton>
                  <DateButton active={calc.compaction === 1.15} onClick={() => setCalc({ ...calc, compaction: 1.15 })}>
                    Lekko +15%
                  </DateButton>
                  <DateButton active={calc.compaction === 1.2} onClick={() => setCalc({ ...calc, compaction: 1.2 })}>
                    Mocno +20%
                  </DateButton>
                </div>
              </div>
              {calcResult && (
                <div className="mt-6 rounded-3xl bg-emerald-50 p-5">
                  <div className="text-lg font-bold">Powierzchnia: {calcResult.m2.toFixed(2)} m²</div>
                  <div className="text-lg font-bold">Objętość: {calcResult.m3.toFixed(2)} m³</div>
    ...
    <article className="group overflow-hidden rounded-[2rem] bg-white shadow-sm ring-1 ring-stone-200 transition hover:-translate-y-1 hover:shadow-xl hover:shadow-stone-300/40">
      ...
      <div className="p-4">
        ...
        <div className="mt-3 flex items-end justify-between gap-3 rounded-2xl bg-stone-50 p-3">
          <div>
            <div className="text-xs font-bold text-zinc-500">Netto / {product.unit}</div>
            <div className="text-2xl font-black">{currency(product.priceNet)}</div>
          </div>
        ...
        <div className="mt-4 flex items-center gap-3">
          <div className="flex min-h-14 flex-1 items-center justify-between rounded-2xl border border-stone-200 p-2">
            <button
              type="button"
              onClick={() => setQty(product.id, Math.max(1, Number(qty || 1) - 1))}
...
function Panel({ children }) {
  return <section className="rounded-[2rem] bg-white p-5 shadow-sm ring-1 ring-stone-200 sm:p-6">{children}</section>;
}

function SectionIntro({ eyebrow, title, text }) {
  return (
    <div>
      <p className="text-sm font-black uppercase tracking-wide text-emerald-800">{eyebrow}</p>
      <h2 className="mt-1 text-3xl font-black tracking-tight">{title}</h2>
      <p className="mt-2 max-w-2xl text-zinc-600">{text}</p>
    </div>
  );
}
  ...
  return (
    <button
      ...
      className={`min-h-44 rounded-3xl border p-6 text-left transition hover:-translate-y-1 hover:shadow-xl ${
        active ? "border-emerald-700 bg-emerald-50 shadow-lg shadow-emerald-900/10" : "border-stone-200 bg-stone-50"
      }`}
    >
...
function StepActions({ onBack, onNext, backLabel, nextLabel }) {
  return (
    <div className="mt-6 flex justify-between gap-3">
      <button
        ...
        onClick={onBack}
        disabled={!onBack}
        className={`inline-flex min-h-14 items-center gap-2 rounded-2xl px-5 font-black ${
          onBack ? "bg-stone-100 text-zinc-700" : "invisible"
        }`}
      >
        <ArrowLeft size={18} /> {backLabel || "Wstecz"}
      </button>
      <button
        type="button"
        onClick={onNext}
        className="inline-flex min-h-14 items-center gap-2 rounded-2xl bg-emerald-800 px-6 font-black text-white shadow-lg shadow-emerald-900/15"
      >
        {nextLabel || "Dalej"} <ArrowRight size={18} />
      </button>
    </div>
  );
}
...
function Input({ error, className = "", ...props }) {
  return (
    <input
      {...props}
      className={`min-h-14 w-full rounded-2xl border bg-white p-4 outline-none transition focus:border-emerald-700 focus:ring-4 focus:ring-emerald-100 ${
...
function DateButton({ active, children, ...props }) {
  return (
    <button
      type="button"
      {...props}
      className={`min-h-12 rounded-2xl px-4 font-black transition ${
        active ? "bg-emerald-800 text-white" : "bg-stone-100 text-zinc-700 hover:bg-stone-200"
      }`}
    >
      {children}
    </button>
...
function MiniProof({ icon, title, text }) {
  return (
    <div className="flex items-center gap-3 rounded-2xl bg-white/80 p-3 shadow-sm ring-1 ring-stone-200">
      <div className="text-emerald-800">{icon}</div>
      <div>
...
function SummaryItem({ item }) {
  return (
    <div className="flex items-center gap-3 rounded-2xl border border-stone-200 p-3">
...
function SummaryTotals({ zone, totals }) {
  return (
    <div className="mt-5 rounded-3xl bg-stone-50 p-4 text-sm">
      ...
      <div className="mt-2 flex justify-between">
        ...
        <b>{!totals.hasDeliverySelected ? "wybierz dostawę" : zone?.price === null ? "do wyceny" : currency(totals.deliveryNet)}</b>
      </div>
      <div className="mt-2 flex justify-between">
        <span>VAT 23%:</span>
...
function ContactSection({ whatsappText }) {
  return (
    <section id="kontakt" className="scroll-mt-28 mx-auto max-w-7xl px-4 pb-20 lg:px-8">
      <div className="rounded-[2rem] bg-emerald-950 p-6 text-white shadow-sm">
        <h2 className="text-2xl font-black">Potrzebujesz pomocy?</h2>
    ...
    <div className="fixed inset-x-0 bottom-0 z-[90] border-t border-stone-200 bg-white p-4 shadow-2xl">
      <div className="mx-auto flex max-w-7xl flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        <div className="text-sm leading-6 text-zinc-600">
          ...
          <br />
          Używamy cookies technicznych do działania strony. Opcjonalne cookies analityczne uruchomimy tylko po Twojej zgodzie.
